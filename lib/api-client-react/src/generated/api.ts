...
export const activateSubscription = async (
  ...
  return customFetch<MySubscription>(getActivateSubscriptionUrl(), {
    ...options,
    method: "POST",
    headers: { "Content-Type": "application/json", ...options?.headers },
 ...
 */
export const getUploadGradesUrl = () => {
  return `/api/grades/upload`;
};

export const uploadGrades = async (
  options?: RequestInit,
): Promise<GradeAnalysisResult> => {
  return customFetch<GradeAnalysisResult>(getUploadGradesUrl(), {
    ...options,
    method: "POST",
  });
};

export const getUploadGradesMutationOptions = <
  TError = ErrorType<ErrorResponse>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
    Awaited<ReturnType<typeof uploadGrades>>,
    TError,
    void,
    TContext
  >;
  ...
  TContext
> => {
  const mutationKey = ["uploadGrades"];
  const { mutation: mutationOptions, request: requestOptions } = options
    ? options.mutation &&
      "mutationKey" in options.mutation &&
      options.mutation.mutationKey
  ...
  const mutationFn: MutationFunction<
    ...
    void
  > = () => {
    return uploadGrades(requestOptions);
  };

  return { mutationFn, ...mutationOptions };
};

export type UploadGradesMutationResult = NonNullable<
  Awaited<ReturnType<typeof uploadGrades>>
>;

export type UploadGradesMutationError = ErrorType<ErrorResponse>;

/**
 * @summary Upload Excel file for grade analysis
 */
export const useUploadGrades = <
  TError = ErrorType<ErrorResponse>,
  TContext = unknown,
>(options?: {
  mutation?: UseMutationOptions<
  ...
  TContext
> => {
  return useMutation(getUploadGradesMutationOptions(options));
};
