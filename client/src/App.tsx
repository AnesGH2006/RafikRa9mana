...
function WhatsAppSupportButton() {
  const SUPPORT_PHONE = "213560000000"; // Replace with actual support number
  const message = encodeURIComponent(
    "مرحباً، أحتاج إلى دعم تقني لمنصة رفيق الرقمنة 🎓"
  );
  const href = `https://wa.me/${SUPPORT_PHONE}?text=${message}`;
  return (
    <motion.div
      className="mx-2 mb-2"
    ...
    >
      <motion.a
        ...
        className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl bg-emerald-600/15 border border-emerald-500/20 hover:bg-emerald-600/25 transition-all"
      >
        <div className="w-6 h-6 rounded-lg flex items-center justify-center shrink-0 bg-[#25D366]">
          {/* WhatsApp icon */}
